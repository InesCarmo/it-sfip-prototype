...
function DiscoverScreen({ workspace, setWorkspace, shortlist, setShortlist, setDetail, navigate }: { workspace: WorkspaceContext; setWorkspace: (v: WorkspaceContext) => void; shortlist: string[]; setShortlist: (v: string[]) => void; setDetail: (v: Opportunity) => void; navigate: (m: Mission) => void }) {
  const [query, setQuery] = useState(workspace.description);
  const recommendations = coreEngine.getContextualRecommendations({ ...workspace, description: query }, 8);
  return <>
    ...
    <div className="search-box"><span>⌕</span><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Descreva a ideia, objetivos, tecnologia, maturidade e parceiros..." /><button className="button" onClick={() => setWorkspace({ ...workspace, title: query.slice(0, 70) || "Análise atual", description: query })}>Analisar</button></div>
    <div className="filter-row"><select value={workspace.area ?? ""} onChange={e => setWorkspace({ ...workspace, area: e.target.value })}><option value="">Todas as áreas</option>{coreEngine.facets.areas.map(value => <option key={value}>{value}</option>)}</select><select value={workspace.group ?? ""} onChange={e => setWorkspace({ ...workspace, group: e.target.value })}><option value="">Todos os grupos</option>{coreEngine.facets.groups.map(value => <option key={value}>{value}</option>)}</select></div>
    <div className="context-evidence"><div><small>WORKSPACE</small><strong>{workspace.title}</strong></div><div><small>SINAIS CONSIDERADOS</small><strong>Descrição · área · grupo · programa · investigador · empresa</strong></div><div><small>FONTE</small><strong>tblCalls + tblMatching + tblRadar</strong></div></div>
    <div className="results-heading"><div><span className="section-label">RECOMENDAÇÕES DO CORE ENGINE</span><h2>{recommendations.length} oportunidades com evidência contextual</h2></div><div className="confidence"><span />Ranking recalculado para este Workspace</div></div>
    <div className="opportunity-list">{recommendations.map(({ item, score }) => <div key={item.id}><div className="confidence">Score contextual: {score}</div><OpportunityCard item={item} selected={shortlist.includes(item.id)} onOpen={() => setDetail(item)} onSelect={() => setShortlist(shortlist.includes(item.id) ? shortlist.filter(id => id !== item.id) : [...shortlist, item.id])} /></div>)}</div>
  </>;
}
...
function TrackScreen({ tasks, setTasks }: { tasks: typeof initialTasks; setTasks: (v: typeof initialTasks) => void }) {
  ...
  return <>
    ...
    <div className="tracking-layout"><section className="task-board"><div className="panel-heading"><div><span className="section-label">AÇÕES DA TBLCALLS</span><h2>Prioridades</h2></div><span>{visible.filter(t => !t.done).length} abertas</span></div>{visible.map(task => <label key={task.id} className={`task ${task.done ? "done" : ""}`}><input type="checkbox" checked={task.done} onChange={() => setTasks(tasks.map(t => t.id === task.id ? {...t, done: !t.done} : t))} /><span className="task-check">✓</span><div><strong>{task.label}</strong><small>{task.id} · {task.call}</small></div><Tag tone={task.days !== null && task.days <= 30 ? "warn" : "neutral"}>{task.due}</Tag></label>)}</section><aside className="risk-panel"><span className="section-label">QUALIDADE OPERACIONAL</span><div className="risk"><span>i</span><div><strong>Fonte de ações</strong><p>Campo Ação Recomendada da tblCalls.</p></div></div><div className="risk"><span>!</span><div><strong>Persistência</strong><p>Alterações locais ainda não escrevem no Excel.</p></div></div></aside></div>
  </>;
}
