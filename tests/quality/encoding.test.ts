...
for (const directory of scanRoots) {
  for (const file of await walk(directory)) {
    ...
    content.split(/\r?\n/).forEach((line, index) => {
      const token = suspiciousTokens.find((candidate) => line.includes(candidate));
      if (token) {
        findings.push({
...
assert.equal(findings.length, 0, `Mojibake detected in app/components/lib:\n${findings.map((item) => `${item.file}:${item.line} [${item.token}] ${item.snippet}`).join("\n")}`);

process.stdout.write("Encoding audit passed for app/components/lib.\n");
